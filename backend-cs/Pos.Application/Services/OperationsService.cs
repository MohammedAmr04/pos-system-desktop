...
namespace PosCs.Application.Services
{
    public class OperationsService
    {
        ...
        {
            request = request ?? new CreateInventoryAdjustmentRequest();
            request.Reason = string.IsNullOrWhiteSpace(request.Reason) ? "Inventory count" : request.Reason.Trim();
            return _repo.CreateAdjustment(request, user);
        }
        ...
        {
            if (request == null) throw new DomainValidationException("Notes are required");
            var adjustment = _repo.GetAdjustment(id);
            ...
            request.Notes = request.Notes == null ? string.Empty : request.Notes.Trim();
            return _repo.UpdateAdjustmentNotes(id, request, user);
        }
        ...
        public void DeleteAdjustment(string id, string user) { _repo.DeleteAdjustment(id, user); }
