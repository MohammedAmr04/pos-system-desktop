...
export function BrandsClient({
  ...
  const handleToggleActive = async (brand: Brand) => {
    try {
      await api.brands.update(brand.id, { isActive: !brand.isActive })
                        ...
                        {canUpdate && (
                          <>
                            <Button
                              variant="ghost"
                              size="icon"
                              aria-label={brand.isActive ? t("deactivated") : t("activated")}
                              onClick={() => handleToggleActive(brand)}
                            >
                              {brand.isActive ? <CircleX className="h-4 w-4" /> : <CircleCheck className="h-4 w-4" />}
                            </Button>
                            <Button variant="ghost" size="icon" aria-label={t("editBrand")} onClick={() => openEdit(brand)}>
                              <Pencil className="h-4 w-4" />
                            </Button>
                          </>
                        )}
                        {canDelete && (
                          <Button variant="ghost" size="icon" aria-label={t("confirmDelete")} onClick={() => handleDelete(brand)}>
