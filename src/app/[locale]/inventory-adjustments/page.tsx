...
export default function InventoryAdjustmentsPage() {
  ...
  const [selectedId, setSelectedId] = useState<string | null>(() =>
    typeof window === "undefined"
      ? null
      : window.sessionStorage.getItem("inventory-adjustment-session"),
  );
  const { data, isPending } = useInventoryAdjustments(page);
  const columns = useAdjustmentColumns(setSelectedId);
  ...
  if (selectedId)
    return (
      <InventorySession
        id={selectedId}
        onBack={() => {
          window.sessionStorage.removeItem("inventory-adjustment-session");
          setSelectedId(null);
